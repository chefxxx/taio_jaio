...
struct SI_State
{
  ...
  void serialNextState(const size_t col) { M(R, static_cast<long>(col)) = 1; R++; cols[col] = true; }
  void serialPrevState(const size_t col) { cols[col] = false; R--; M(R, static_cast<long>(col)) = 0; }
